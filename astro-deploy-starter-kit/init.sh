...
install_dependencies() {
    ...
    # Limpiar vulnerabilidades de seguridad (solo para npm)
    if [ "$PACKAGE_MANAGER" == "npm" ]; then
        log_info "Ejecutando npm audit fix para resolver vulnerabilidades..."
        npm audit fix 2>/dev/null || true
        
        # Verificar si hay vulnerabilidades críticas/altas aún pendientes
        REMAINING_VULNS=$(npm audit --json 2>/dev/null | grep -o '"high":[0-9]*' | sed 's/[^0-9]//g')
        if [ -n "$REMAINING_VULNS" ] && [ "$REMAINING_VULNS" -gt 0 ]; then
            log_warning "Hay $REMAINING_VULNS vulnerabilidades altas aún pendientes."
            log_info "Para resolver manualmente, ejecuta: npm audit fix --force"
        fi
    fi
}
...
setup_astro_skill() {
    ...
    echo ""
    echo "  mkdir -p .skills/astro-docs"
    echo "  git clone $SKILL_REPO .skills/astro-docs"
    echo ""
...
# =============================================================================
# PASO 8: INSTALAR SKILL EN HOME
# =============================================================================

setup_home_skill() {
    log_step "Paso 8: Instalar Skill en \$HOME/.qwen/skills (Opcional)"
    
    echo "¿Deseas instalar la skill también en \$HOME/.qwen/skills/astro-docs?"
    echo ""
    echo "  1) Sí"
    echo "  2) No"
    echo ""
    ...
    case $OPTION in
        1)
            log_info "Instalando skill en \$HOME/.qwen/skills/astro-docs..."
            
            HOME_SKILL_DIR="$HOME/.qwen/skills/astro-docs"
            SKILL_REPO="https://github.com/asachs01/astro-docs-skill"
            
            # Crear directorio si no existe
            mkdir -p "$HOME_SKILL_DIR"
            
            # Copiar desde .skills/ local si existe
            if [ -d ".skills/astro-docs" ] && [ -f ".skills/astro-docs/SKILL.md" ]; then
                log_info "Copiando skill desde .skills/astro-docs..."
                cp -r ".skills/astro-docs"/* "$HOME_SKILL_DIR/" 2>/dev/null || {
                    log_warning "No se pudo copiar desde .skills/"
                }
                
                if [ -f "$HOME_SKILL_DIR/SKILL.md" ]; then
                    log_success "Skill instalada en \$HOME/.qwen/skills/astro-docs"
                    return 0
                fi
            fi
            
            # Clonar desde GitHub si no se pudo copiar
            if command -v git &> /dev/null; then
                log_info "Clonando skill desde GitHub..."
                rm -rf "$HOME_SKILL_DIR"
                git clone "$SKILL_REPO" "$HOME_SKILL_DIR" 2>/dev/null || {
                    log_error "No se pudo clonar desde GitHub"
                    return 1
                }
                
                if [ -f "$HOME_SKILL_DIR/SKILL.md" ]; then
                    log_success "Skill instalada exitosamente en \$HOME/.qwen/skills/astro-docs"
                    return 0
                fi
            else
            ...
            ;;
        2)
            log_info "Instalación en \$HOME/.qwen/skills omitida"
            return 0
            ;;
        *)
